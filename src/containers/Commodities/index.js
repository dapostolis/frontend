...
function Commodities({classes}) {
  ...
  function handleSubmit(form, commodityDict) {
    ...
    // Validation
    if (!form.instrument) {
      enqueueSnackbar('Commodity field is required', {variant: 'error'});
      return;
    }
    ...
    request
      .post(`${API}alchemist/commodity`, data)
      .then(({data: {returnobject: body}}) => {
        ...
        if (body.parsedReturns.default && body.parsedReturns.default.length > 0) {
          ...
          chart.push({
            ...
            name: commodityDict[convertStringToMachineName(form.instrument)],
            data: chartDefault
          });
  ...
  return (
    ...
    <div className={classes.container}>
      ...
      <SidebarWrapper id="commodities-sidebar-panel" className={classes.sidebar}>
        <Form loading={loading} onHandleSubmit={handleSubmit}/>
      </SidebarWrapper>

      <MainBareWrapper>

        <MainHeaderWrapper id="commodities-top-main">
          <Heading title="Commodities"/>
        </MainHeaderWrapper>

        {/*<InfoBox type="warning" show={isSchedulerRunning}>Alchemist is updating data. xxxx</InfoBox>*/}
