...
function AssetClassOverview({classes, dyLoading, overviewlist, onHandleRearrangeOverview, onHandleChangeOverviewField, onHandleDeleteOverviewField, onHandleAutoSave}) {

  const [moveFrom, setMoveFrom] = useState(-1); // -1 means no item has been selected
  ...
  function handleMouseOverMove(moveTo) {
    ...
    onHandleRearrangeOverview(moveFrom, moveTo, setMoveFrom);
  }

  function handleDelete(key) {
    onHandleDeleteOverviewField(key);
    setTimeout(onHandleAutoSave, 500); // todo - change logic. useEffect
  }
  ...
  return (
    <div className={classes.tableRoot}>
      ...
      <div className={classes.table}>
        ...
        <div className={classnames(classes.tBody, {'start-sort': moveFrom > -1})}>
          {overviewlist && overviewlist.sort(compare).map((overview, key) => {
            ...
            return (
              <div
                key={key}
                className={classnames(classes.tRow, 'action-parent', {'progress': dyLoading}, {active: key === moveFrom})}
                onClick={handleStopMove}
                onMouseOver={() => handleMouseOverMove(key)}
              >
                {/*<Tooltip title={moveFrom === -1 ? 'Click to sort' : 'Move your mouse and click to finish'} placement="bottom-start">*/}
                {/*<SwapVertIcon className={classnames(classes.swap, 'swap-sort')} onClick={() => handleMove(key)}/>*/}

                <Typography component="div" className={classnames(classes.tCell, classes.tCell1)}>
                  {/*Disable <EditInput/> in order to avoid simultaneous events when we are onRearrangement state*/}
                  {key === moveFrom
                    ? overview.assetclass
                    : <EditInput
                      forceEmptyValue
                      fieldValue={overview.assetclass}
                      defaultValue=""
                      css={{
                        result: classes.resultAssetClass,
                        control: classes.assetClassInput,
                      }}
                      disabled={dyLoading}
                      onChange={onHandleChangeOverviewField(key, 'assetclass')}
                ...
                <Typography component="div" className={classnames(classes.tCell, classes.tCell2)}>
                  ...
                  <div className={classes.overviewText}>
                    <EditSelect
                      isOpened={true}
                      options={OVERVIEW_VIEW_LIST}
                      fieldValue={overview.view}
                      defaultValue=""
                      css={{result: classes.resultView}}
                      disabled={dyLoading}
                      onChange={onHandleChangeOverviewField(key, 'view')}
                      onClose={onHandleAutoSave}
                    />
                  </div>
                </Typography>
                ...
                {moveFrom === -1 ?
                  <div className={classnames('action')}>
                    <Typography component="div" onClick={() => handleDelete(key)}>Remove <DeleteIcon fontSize="inherit"/></Typography>
                    <Typography component="div" onClick={event => handleMove(event, key)}>Move <ControlCameraIcon fontSize="inherit"/></Typography>
                  </div> : ''}
              </div>
            );
          })}
