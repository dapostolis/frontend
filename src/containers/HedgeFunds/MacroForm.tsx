...
import {IFundsSpecificStateFields, IGlobalStateFields, IKeyValue, IAvailableFunds, IFactor, IFactorSelected, TSelectedFactors} from "./Interface/IHedgeFundInput";
    ...
    <form className={classes.form} autoComplete="off" onSubmit={event => event.preventDefault()}>
      ...
      <div className={classes.gapContainer}>
        ...
        {/*<FormControl className={classnames(classes.formControl, classes.inputFullWidth)}>
          ...
          >
            <option value=""></option>
            {Object.keys(lists.types).map((type: string, key: number) => <option key={key} value={type}>{type}</option>)}
          </Select>
        </FormControl>*/}
